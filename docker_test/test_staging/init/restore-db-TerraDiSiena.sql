-- restore-dq.sql for TERRADISIENA

RESTORE DATABASE [TERRADISIENA]
FROM DISK = '/var/opt/mssql/backup/2_TERRADISIENA.bak'
WITH MOVE 'ECCHER' TO '/var/opt/mssql/TERRADISIENA.mdf',
     MOVE 'ECCHER_log' TO '/var/opt/mssql/TERRADISIENA_log.ldf',
     REPLACE,
     STATS = 5;
GO
...
-- (optional) ALTER DATABASE [TERRADISIENA] SET READ_WRITE WITH ROLLBACK IMMEDIATE;

-- 1) Enable CT at database level (or update retention/cleanup if already ON)
IF NOT EXISTS (
    SELECT 1
    FROM sys.change_tracking_databases
    WHERE database_id = DB_ID(N'TERRADISIENA')
)
BEGIN
    ALTER DATABASE [TERRADISIENA]
